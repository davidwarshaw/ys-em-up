...
function stateMachine(scene, character, player, bullets, map) {
  ...
  switch (character.ai.state) {
    ...
    case "swoop": {
      ...
      // console.log(`map.isBackground(character): ${map.isBackground(character)}`);
      if (map.isBackground(character)) {
        character.ai.passedOverMap = true;
      }
      if (character.ai.passedOverMap && !map.isBackground(character)) {
